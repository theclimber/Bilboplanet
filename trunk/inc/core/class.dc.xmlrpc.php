...
class dcXmlRpc extends xmlrpcIntrospectionServer
{
	...
	private function newPost($blog_id,$user,$pwd,$content,$struct=array(),$publish=true)
	{
		...
		elseif ($cur->post_type == 'page')
		{
			...
			$this->core->blog->settings->post_url_format = '{t}';
	...
	{
		$timezone = 0;
		if ($this->core->blog->settings->blog_timezone) {
			$timezone = dt::getTimeOffset($this->core->blog->settings->blog_timezone)/3600;
		}
		...
		$res = array (
			...
			'date_format' => array (
				'desc' => 'Date Format',
				'readonly' => false,
				'value' => $this->core->blog->settings->date_format
			),
			'time_format' => array (
				'desc' => 'Time Format',
				'readonly' => false,
				'value' => $this->core->blog->settings->time_format
			)
		);
	...
	private function deleteCategory($user,$pwd,$cat_id)
	{
		...
		if ($c->isEmpty()) {
			throw new Exception(__('This category does not exists.'));
		}
		$cat_id = $c->cat_id;
		unset($c);
		...
		foreach ($options as $name => $value)
		{
			...
			switch ($name)
			{
				...
				case 'blog_tagline':
					...
					break;
				case 'date_format':
					$this->core->blog->settings->put('date_format',$value);
					$done[] = $name;
					break;
				case 'time_format':
					$this->core->blog->settings->put('time_format',$value);
					$done[] = $name;
					break;
			}
		}
