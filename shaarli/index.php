...
class linkdb implements Iterator, Countable, ArrayAccess
{
    ...
    public function filterWeek($week)
    {
        $filtered=array();
        foreach($this->links as $l)
        {
			$linktime = mktime(0,0,0, substr($l['linkdate'],4,2), substr($l['linkdate'],6,2), substr($l['linkdate'],0,4));
			$linkpubdate = Date('Ymd',$linktime + 3600*24*(8-Date('N',$linktime)));
            if ($linkpubdate == $week) $filtered[$l['linkdate']] = $l;
        }
        ksort($filtered);
        return $filtered;
    }
